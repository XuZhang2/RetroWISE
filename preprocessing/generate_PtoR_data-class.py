...
"""multiprocess"""
def preprocess(save_dir, reactants, products,set_name, augmentation=1, class_list=[], reaction_types=None,root_aligned=True,character=False, processes=-1):
    """
    ...
    }  for i,j,k in zip(reactants,products,class_list)]
    src_data = []
    tgt_data = []
    class_data = []
    skip_dict = {
    ...
    try:
        results = run_imap_mp(multi_process, data, 8, True)
    ...
    for result in tqdm(results):
        ...
        class_data.extend(result['class'])
    ...
    if augmentation != 999:
        ...
        with open(
            ...
            for tgt in tgt_data:
                f.write('{}\n'.format(tgt))
        with open(
                os.path.join(save_dir, 'class-{}.txt'.format(set_name)), 'w') as f:
        ...
        if data['root_aligned']:
            ...
            if augmentation == 999:
                ...
                if reversable:
                    times = int(times / 2)
            # candidates = []
            #print(reactant, times, flush=True)
            for k in range(times):
                ...
                return_status['class'].append(data['class'])
...
if __name__ == '__main__':
    ...
    assert args.dataset in ['USPTO_50K', 'USPTO_full','USPTO-MIT', 'USPTO_extra_mit', 'USPTO_extra_full']
    print(args)
    if args.test_only:
        datasets = ['test']
    ...
    if args.dataset == "USPTO-MIT":
        ...
        datadir = './dataset/{}'.format(args.dataset)
        savedir = './dataset/{}_PtoR_aug{}_seed{}_class'.format(args.dataset, args.augmentation, args.seed)
        ...
        for i, data_set in enumerate(datasets):
            ...
            print("Total Class Size", len(class_list))
            ...
            src_data, tgt_data = preprocess(
                ...
                data_set,
                args.augmentation,
                class_list,  # add class_list as an argument
                reaction_types=None,
                root_aligned=not args.canonical,
                character=args.character,
                processes=args.processes,
